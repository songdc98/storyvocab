(function () {
  ...
  function dayTwoLockedLibraryStory(items, density) {
    ...
    const paragraphs = [
      `奖学金颁奖夜，图书馆忽然落锁，馆长只来得及 ${c(0)} 她一句：沿着地上的红色 ${c(1)} 走，答案不在告示牌上，而是藏在告示牌下面：the answer is ${c(2)} the notice board。门口的牌子写着 ${c(3)} enter ${c(4)} permission，而第七层旧书架里夹着一张撕下来的 ${c(5)}。她按照 ${c(6)} list 找到一份发黄的 ${c(7)}，决定 ${c(8)} 一次。窗外的雨像一片黑色的 ${c(9)}，她从未 ${c(10)} 过这种安静：纸边被人整齐 ${c(11)} 开，旁边摆着三 ${c(12)} 钥匙。一个 ${c(13)} donor 的名字被圈起来，下面写着一个疯狂的 ${c(14)}，以及所有可疑 ${c(15)} 的编号。她突然 ${c(16)} 地意识到，今晚热得发闷的大厅并不只是 ${c(17)}；她 ${c(18)} leave this ${c(19)} until she understood the first clue.`,
      ...
      `墙上的符号开始 ${c(100)} 她去洗手间。水槽下的管道还在 ${c(101)} 水，镜子背面藏着一份 ${c(102)} 转让书。保安说 the guard had been ${c(103)} for an hour；蓝图上有 a row of abandoned ${c(104)} 被红线圈住，旁边的监控照片里，一队 ${c(105)} 正从奖杯底座爬过。档案还提到 a closed ${c(106)} hearing 和一场假装成意外的 ${c(107)}。地球仪旁边的标签写着 ${c(108)} axis，旁边一个玻璃 ${c(109)} 里装着 the ${c(110)} of every secret donation。名单上只 ${c(111)} 三个人，证词却 ${c(112)} 了第四个人存在。她想起父亲，低声喊了一句 ${c(113)}。灯光的 ${c(114)} 变得像鼓点，雨的 ${c(115)} 正好盖住脚步声。为了 ${c(116)} 被发现，她把文件分成几个 ${c(117)}，用最后一点 ${c(118)} 把金属 ${c(119)} 插进锁孔。`,
      ...
      `她把线索 ${c(140)} 到一起，用绳结 ${c(141)} 在桌脚上。The ${c(142)} did not match the receipts，最后一盏 ${c(143)} 却突然亮起，照出地下室的旧 ${c(144)} 轨道。冷风 ${c(145)} 过来，像有人仍在里面呼吸。她保持 ${c(146)}，因为校长已经 ${c(147)} 她的声音；下一步必须 ${c(148)} 得像一场演讲。线索的 ${c(149)} 竟是一张 ${c(150)} 表：屏幕 ${c(151)} 上从原始 ${c(152)} 到现代系统的记录全被 ${c(153)}。外面停着几辆 ${c(154)}，资料却写着 a fake ${c(155)} in ${c(156)}、a hive of ${c(157)} 和 a cancelled ${c(158)} plan。看似荒诞的 ${c(159)} coordinates，其实是隐藏资金流的入口。`,
