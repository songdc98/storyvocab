(function () {
  ...
  const PHONETIC_TOKEN_OVERRIDES = {
    a: "/ə/",
    about: "/əˈbaʊt/",
    ...
    at: "/æt/",
    attached: "/əˈtætʃt/",
    authentication: "/ɔːˌθen.təˈkeɪ.ʃən/",
    back: "/bæk/",
    backlog: "/ˈbæk.lɑːɡ/",
    ...
    control: "/kənˈtroʊl/",
    conversion: "/kənˈvɝː.ʒən/",
    cost: "/kɑːst/",
    could: "/kʊd/",
    criteria: "/kraɪˈtɪr.i.ə/",
    customer: "/ˈkʌs.tə.mɚ/",
    dashboard: "/ˈdæʃ.bɔːrd/",
    data: "/ˈdeɪ.t̬ə/",
    date: "/deɪt/",
    day: "/deɪ/",
    deadline: "/ˈded.laɪn/",
    ...
    deep: "/diːp/",
    delay: "/dɪˈleɪ/",
    deliverable: "/dɪˈlɪv.ɚ.ə.bəl/",
    dependency: "/dɪˈpen.dən.si/",
    ...
    option: "/ˈɑːp.ʃən/",
    order: "/ˈɔːr.dɚ/",
    oriented: "/ˈɔːr.i.en.tɪd/",
    outage: "/ˈaʊ.t̬ɪdʒ/",
    outcome: "/ˈaʊt.kʌm/",
    overview: "/ˈoʊ.vɚ.vjuː/",
    owner: "/ˈoʊ.nɚ/",
    ...
    proactive: "/proʊˈæk.tɪv/",
    procurement: "/prəˈkjʊr.mənt/",
    proposal: "/prəˈpoʊ.zəl/",
    proposition: "/ˌprɑː.pəˈzɪʃ.ən/",
    ...
    reimbursement: "/ˌriː.ɪmˈbɝːs.mənt/",
  ...
  function mappedTokenPhonetics(tokens) {
    return tokens
      .map((token) => PHONETIC_TOKEN_OVERRIDES[token.toLowerCase()] || `/${token}/`)
      .join(" ");
  }

  function pronunciationTitle(item, shown = displayWord(item)) {
    const phrase = phraseFor(item);
    const basePhonetic = normalizePhonetic(item.phonetic);
    if (phrase && basePhonetic) {
      const shownTokens = pronunciationTokens(shown);
      const rootTokens = pronunciationTokens(item.word);
      const extraTokens = shownTokens.slice(rootTokens.length);
      const extraPhonetics = mappedTokenPhonetics(extraTokens);
      return [basePhonetic, extraPhonetics].filter(Boolean).join(" ");
    }
    if (basePhonetic) return basePhonetic;
    const tokens = pronunciationTokens(shown);
    return tokens.length ? mappedTokenPhonetics(tokens) : "美音可朗读";
  }
  ...
  function chip(item) {
    ...
    const menuTitle = `${pronunciationTitle(item, shown)} · ${item.zh || "常用词"}`;
  ...
  function campusWordRole(item) {
    ...
    const zh = String(item.zh || "").toLowerCase();
    if (/^\d+$/.test(word) || zh.includes("num.")) return "number";
    if (/^[a-z]$/.test(word)) return "code";
    if (/'s$/.test(word) && !["it's", "that's", "what's", "there's", "he's", "she's"].includes(word)) return "possessive";
    if (word.includes("'")) return "function";
    if (zh.includes("prep.") || zh.includes("conj.") || zh.includes("aux.")) return "function";
    if (zh.startsWith("n.")) return "noun";
    if (zh.startsWith("a.") || zh.startsWith("adj.")) return "adjective";
    if (zh.startsWith("adv.")) return "adverb";
    if (zh.startsWith("vt.") || zh.startsWith("vi.") || /^v[.\s]/.test(zh)) return "verb";
    if (zh.includes("n.")) return "noun";
    if (zh.includes("a.") || zh.includes("adj.")) return "adjective";
    if (zh.includes("adv.")) return "adverb";
    if (zh.includes("vt.") || zh.includes("vi.") || zh.includes("过去式") || zh.includes("过去分词") || zh.includes("第三人称")) return "verb";
    return "noun";
  }

  function campusArticleFor(word) {
    const raw = String(word || "").trim();
    if (!raw || /^[A-Z]/.test(raw) || /^\d+$/.test(raw) || /s$/i.test(raw) && !/ss$/i.test(raw)) return "the";
    return /^[aeiou]/i.test(raw) ? "an" : "a";
  }

  function campusNounPhrase(item, wordHtml) {
    const raw = String(displayWord(item) || item.word || "");
    if (/^[A-Z]/.test(raw) || /^\d+$/.test(raw)) return wordHtml;
